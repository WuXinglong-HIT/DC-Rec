...
class CombineGraph(Module):
    ...
    def forward(self, inputs, adj, mask_item, item, seq, lengths,order):
        ...
        #h_local = self.local_agg(h_local, adj, mask_item)
        #h_local = self.local_agg(h_local, adj, mask_item)
        # global
        ...
        #output = 15.0/8.0*h_local + 1.0/8.0*h_global
        #output = 7.0/4.0*h_local + 1.0/4.0*h_global
       # output=h_local
       ...
       # b_t= 1/2.0*c_local+ 1/2.0*c_global #b_t是要找的监督向量
        b_t= c_local
...
def getDCG(scores):
    #print(np.divide(np.power(2, scores) - 1, np.log2(np.arange(scores.shape[0], dtype=np.float32) + 2)) )
    return np.sum(
